...
public enum BenchmarkReportRenderer {
    ...
    public static func render(
        ...
        markdown.append("## Pipeline Cleanup Quality")
        markdown.append("- Source: `\(pipelineLabel)`")
        markdown.append("| Metric | Value |")
        markdown.append("|---|---:|")
        ...
        markdown.append("| Repair Marker Preservation Rate | \(percent(pipelineSummary.repairMarkerPreservationRate)) |")
        markdown.append("| Repair Trigger Detection Rate | \(percent(pipelineSummary.repairResolutionRate)) |")
        markdown.append("| Repair Exact Match Rate | \(percent(pipelineSummary.repairExactMatchRate)) |")
        markdown.append("| Unintended Rewrite Rate | \(percent(pipelineSummary.unintendedRewriteRate)) |")
        markdown.append("| Literal Repair Phrase Preservation Rate | \(percent(pipelineSummary.literalRepairPhrasePreservationRate)) |")
        markdown.append("| Punctuation Artifact Rate | \(percent(pipelineSummary.punctuationArtifactRate)) |")
        markdown.append("")

        markdown.append("## Coordinator End-to-End Replay")
        markdown.append("- Source: `SessionCoordinator` replay observations")
        markdown.append("| Metric | Value |")
        markdown.append("|---|---:|")
        markdown.append("| Command Passthrough Coverage Rate | \(percent(pipelineSummary.commandPassthroughCoverageRate)) |")
        markdown.append("| Command Passthrough Accuracy | \(percent(pipelineSummary.commandPassthroughAccuracy)) |")
        markdown.append("| No-Speech False Insert Rate | \(percent(pipelineSummary.noSpeechFalseInsertRate)) |")
        markdown.append("| p50 Latency (ms) | \(decimal(pipelineSummary.p50LatencyMS)) |")

...
The native CI lane compiles the actual patched CPU and Whisper translation units into isolated allocation-test executables. Four injected allocation failures verify null returns and cleanup; successful controls cover CPU initialization, plan creation and synthetic VAD segment boundaries. The injection code is not linked into the shipped helper.

...
- CodeQL scans the workflow language, Swift app and package, and native C++ compilation. PR scans explicitly disable diff-informed queries so the gate receives full results, including findings outside changed lines. Workflow policy rejects missing or overridden full-scan configuration. The local SARIF gate blocks security severity 7.0 and above and non-security error-level findings, including existing and suppressed findings. Missing or invalid scan evidence also fails. Lower-severity results remain visible in GitHub's Security tab.

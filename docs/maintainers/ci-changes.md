...
This record explains the corrections made while activating the 1.0 pipeline in [PR #16](https://github.com/Ankit-Cherian/steno/pull/16) and the follow-up repairs integrated in [PR #20](https://github.com/Ankit-Cherian/steno/pull/20). Recorded on September 15, 2026 UTC. Earlier failure and local-test records below describe their named revisions; the completed hosted results are recorded first.
...
The three release environments are enabled, restricted to `main`, and require maintainer approval; administrator bypass is disabled. Signing and notarization credentials are confined to the protected `release` environment. The draft and publication environments contain no Apple credentials. Configuration verification does not establish signing, notarization, publication, or native app acceptance; those receipts belong in the [release checklist](../release/1.0-checklist.md).
...
On `043314a`, the [full Security workflow](https://github.com/Ankit-Cherian/steno/actions/runs/34923182707) passed, including the C++ high/critical severity gate after repair of the 61 prior blocking results. Package and hosted-app jobs passed on both macOS 15 and 26. The runtime suite still had three VAD-related timeouts; the diagnostic comparison above identified a VAD worker-count correction. On `e48c48d`, the [Security workflow](https://github.com/Ankit-Cherian/steno/actions/runs/34925598286) and both macOS test matrices passed. Both runtime runs passed the previously failing long transcription cases and 25 of 26 cases overall. The push run failed completed-final network observation; the [PR run](https://github.com/Ankit-Cherian/steno/actions/runs/34925599511/job/104242870245) timed out in cancellation/restart. These results validated the VAD timing improvement but left CI blocked at that revision. The follow-up corrections and completed hosted runs are recorded above. Signing, notarization, release publication, and manual app acceptance require separate evidence in the [1.0 release checklist](../release/1.0-checklist.md).

        ...
        VStack(alignment: .leading, spacing: 20) {
            settingsCard("Shortcuts") {
                ...
                VStack(alignment: .leading, spacing: StenoDesign.xxs) {
                    Picker("Hands-free key", selection: handsFreeKeyBinding) {
            ...
            settingsCard("While recording") {
                settingsToggle(
                    "Show live transcript",
                    description: "Preview your words in the recording overlay. Recent words may change; only completed dictation is typed and saved.",
                    isOn: $preferences.dictation.showLiveTranscriptWhileRecording
                )
                Divider()
                settingsToggle(
                    "Use nearby text for continuation",
                    description: "Uses a small amount of nearby text on this Mac to match spacing and capitalization. Nearby text is never saved.",
                    isOn: $preferences.dictation.useNearbyTextForContinuation
                )
                Text("Say “lowercase” before a phrase to start it with a lowercase letter. Say “literal lowercase” to keep that word in the transcript.")
                    .font(StenoDesign.caption())
                    .foregroundStyle(StenoDesign.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
